...
int maintest()
{
...

	{
		...
		XmlTest("First child exists & is a comment.", true, (node != 0
				&& node->ToComment()));
		node = node->NextSibling();
		XmlTest("Sibling element exists & is an element.", true, (node != 0
				&& node->ToElement()));
		...
		count = 0;
		for (node = doc.IterateChildren(0); node; node = doc.IterateChildren(
				node)) {
			count++;
		}
		...
		count = 0;
		for (element = todoElement->FirstChildElement(); element; element
		...
		count = 0;
		for (node = todoElement->FirstChild("Item"); node; node
				= node->NextSibling("Item")) {
			count++;
		}
		...
		count = 0;
		for (node = todoElement->LastChild("Item"); node; node
				= node->PreviousSibling("Item")) {
			count++;
		}
...

	{
		...
		TiXmlHandle
				textHandle = docHandle.FirstChildElement("room").ChildElement(
						"door", 0).FirstChild();
	...
	printf("\n** Copy and Assignment **\n");
	{
		...
		XmlTest("Copy/Assign: element assign #3.", true, (0
				== elementAssign.Attribute("foo")));
	...
	// InsertBeforeChild and InsertAfterChild causes crash.
	{
		...
		TiXmlNode* childNode1 =
				parent.InsertBeforeChild(childNode0, childText1);

		XmlTest("Test InsertBeforeChild on empty node.", (childNode1
				== parent.FirstChild()), true);
	}

	{
		...
		XmlTest("Test InsertAfterChild on empty node. ", (childNode1
				== parent.LastChild()), true);
	}
...

	{
		...
		XmlTest("White space condensed.",
				"This has leading and trailing space", text->Value());
		...
		text = docH.FirstChildElement("element").Child(0).Text();
		XmlTest("White space condensed.",
				"This has leading, trailing, and internal space", text->Value());
	}
...

	{
		...
		XmlTest("Document only at top level.", xml.ErrorId(),
...

	{
		...
		XmlTest("Comments ignore entities.",
				" declarations for <head> & <body> ", c0->Value(), true);
...

	{
		...
		TiXmlNode* ele = 0;
		while ((ele = xml.FirstChildElement("Parent")->IterateChildren(ele))
				!= 0) {
			++count;
		}
